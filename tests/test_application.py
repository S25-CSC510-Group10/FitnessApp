...
def test_display_profile_invalid(client):
    ...
    # Assertions
    assert b"450" not in response.data  # Check that there is not a weight inside the response
...
def test_user_profile_valid(client):
    ...
    response = client.post("/user_profile",
                           data = {
                               "weight": "375",
                               "height": "500",
                               "goal": "Weight Loss",
...
def mock_smtp(monkeypatch):
    ...
    monkeypatch.setattr("application.smtplib.SMTP_SSL", lambda host, port: MockSMTP(host, port))
